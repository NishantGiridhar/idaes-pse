...
def calculate_scaling_factors(b):
    ...
    sf_mf = {}
    for j, v in b.mole_frac_comp.items():
        sf_mf[j] = iscale.get_scaling_factor(v, default=10, warning=True)
    
    for j,v in b.flow_mol_comp.items():
        if gsf(v) is None:
            ssf(v,sf_flow*sf_mf[j])
        
    sf_pf = {}
    for p, v in b.phase_frac.items():
        sf_pf[p] = gsf(v, default=1)
        
    for (p,j), v in b.mole_frac_phase_comp.items():
        if gsf(v) is None:
            ssf(v,sf_pf[p]*sf_mf[j])
        
    for p, v in b.flow_mol_phase.items():
        if gsf(v) is None:
            ssf(v,sf_flow*sf_pf[p])
    
    for (p,j), v in b.flow_mol_phase_comp.items():
        if gsf(v) is None:
            ssf(v,sf_flow*sf_pf[p]*sf_mf[j])
